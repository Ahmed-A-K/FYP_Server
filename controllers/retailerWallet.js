const RetailerWallet = require("../models/retailerWallet");
...
exports.createReciept = async (req, res) => {
  ...
  const receipt = await RetailerWallet({
    email,
    price,
  });
  await receipt.save();
...
exports.uploadReciept = async (req, res) => {
  ...
  try {
    ...
    const updatedUser = await RetailerWallet.findByIdAndUpdate(
      filter,
      { receipt: result.url },
      { new: true }
    );
...
exports.showReciept = async (req, res) => {
  const result = await RetailerWallet.find({});
...
exports.updateStatusBill = async (req, res) => {
  ...
  const updatedStatus = await RetailerWallet.findOneAndUpdate(filter, update, {
    new: true,
  });
  res.json({ success: true });
};
